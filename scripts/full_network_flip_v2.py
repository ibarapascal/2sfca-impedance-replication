"""
xx-impedance-standard 正式计算 v2 (closure): 路网 2SFCA × (6 手选 + 2 経験反演) 規格 × 3 都市圏

v1 との差分: ①SPECS に経験規格 2 本追加(パラメータは decay_inversion.json から読込)
  ②主翻转指标は手选 6 規格のみ(v1 可比), pairwise は全 8(経験規格 closure 対照)
  ③距離対 npz 保留 ④出力 full2/
...
# ②反演产物から経験規格パラメータ読込（可回溯: decay_inversion.json が SSOT）
_inv = json.load(open(os.path.expanduser(
    "~/Documents/codebase/impedance-pilot/inversion/decay_inversion.json")))
_wfits = _inv["自宅－私事_walk"]["fits"]
EMP_BETA = _wfits["negexp_tail3km"]["params"][1]   # ~0.00098 /m
EMP_ALPHA = _wfits["power_tail3km"]["params"][1]   # ~4.06
...
def w_emp_negexp(d): return np.exp(-EMP_BETA * d)
def w_emp_power(d):  return (np.clip(d, 125.0, None) / 125.0) ** -EMP_ALPHA
...
SPECS = [("negexp_b001", w_negexp_slow), ("negexp_b003", w_negexp_fast),
         ...
         ("emp_negexp_walk", w_emp_negexp), ("emp_power_walk", w_emp_power)]
NAMES = [n for n, _ in SPECS]
NS = len(SPECS)
N_HAND = 6  # 前6=手选规格; 后2=経験反演規格（closure 対照）
...
def flip_metrics(A, pop):
    """主翻转指标只在前 N_HAND 个手选规格上计算(与 run1 可比);
    pairwise/Spearman 覆盖全 8 规格(含経験規格 closure 対照)。"""
    res = {}
    for nc in (3, 5, 10):
        Qa = classify(A, nc)
        Q = Qa[:N_HAND]
        ...
        if nc == 5:
            Q5, qr5 = Qa, qmax - qmin  # Q5 全 8 規格(pairwise 用), qr5 手选 6
            bottom = Q == 0
...
def run_region(region, benchmark=False):
    ...
    # v2: 距离对保留（後続 spec 追加の再計算回避; 磁盘 ~4GB 已确认可容纳）
    log(f"=== {region} done ===")
